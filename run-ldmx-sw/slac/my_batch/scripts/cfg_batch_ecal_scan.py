...
if len(sys.argv)>0:
     seedNo = int(args[0])
if len(args)>1:
     if args[1].endswith('.root'):
          outname = args[1]
     else:
          outdir = args[1]
          outname = outdir+'/'+outname

print('Writing output file to:',outname)

p=ldmxcfg.Process("v12")
print('Setting random seed to:',seedNo)
p.run = seedNo
...
#p.maxEvents = 10*1000
p.maxEvents = 10*1000
with open(outdir+'/parameterDump.json', 'w') as outfile:
