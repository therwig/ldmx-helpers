...
#   output_hists    ==> output histogram root file (name and extension)
#
...
def write_config( template_config, outPrefix, job_id, inputFile, newSeed=False ):
    ...
    if ( inputFile == "" ) :
        ...
        totalPrefix += str(job_id) + '_' + inputFile[:-5] #remove .root
    else :
    ...
    logging.info('Saving histogram output to %s' % output_hists )
...
#       --histOut   (optional) full path to directory to copy historgram output file to - must already exist
...
def main():
    ...
    parser.add_argument('--envScript', 
            default='/nfs/slac/g/ldmx/users/eichl008/ldmx-env.sh', #TODO make default env script distribution dependent
    ...
    if os.access('/scratch/', os.W_OK): 
        ...
        scratch_dir = '/nfs/slac/g/ldmx/production/scratch/%s' % os.environ['USER']
    ...
    os.system( "echo Diagnostic info: >> {}".format(logout) )
    os.system( "env >> {}".format(logout) )
    ...
    if args.histOut :
        if os.path.isfile( histsf ) :
            ...
            logging.info('Copying hist file to %s' % histOutputDir)
            if not args.test :
                logging.info('cp %s %s' % ( histsf , os.path.join( histOutputDir , histsf ) ) )
                os.system('cp %s %s' % ( histsf , os.path.join( histOutputDir , histsf ) ) )
        else :
            logging.info('No hist file to copy.')
    else :
        logging.info( 'Not saving histogram file (if it exists).' )
